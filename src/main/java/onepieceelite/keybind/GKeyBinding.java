...
package onepieceelite.keybind;

import org.lwjgl.glfw.GLFW;

import onepieceelite.procedures.GOnKeyPressedProcedure;
...
public class GKeyBinding extends OnePieceEliteModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	public GKeyBinding(OnePieceEliteModElements instance) {
		super(instance, 11);
	...
	public void initElements() {
		keys = new KeyBinding("key.one_piece_elite.g", GLFW.GLFW_KEY_G, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					OnePieceEliteMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
		...
		if (type == 0) {
			{
				...
				GOnKeyPressedProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
